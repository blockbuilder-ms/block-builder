...
<div x-action="toggle" x-options='{"toggle":["translate-y-10", "opacity-0"]}' class="col-12 bg-white flex flex-wrap">
    ...
    <div class="relative w-full">
        <ul x-container class="tabs hidden translate-y-10 opacity-0 duration-200 ease-in-out transition-all bg-light z-50 shadow-sm absolute top-0 flex flex-wrap w-full">
            ...
            ?>
                <li class="w-full">
                    <a x-action="tab" <?php if ($initial) : ?> x-initial="true" <?php endif; ?> href="#colors-<?php echo $name ?>-settings" class="tab text-xs text-decoration-none flex py-4 px-4 cursor-pointer border-b transition-opacity duration-200 ease-in-out">
...
<div class="col-12">
    ...
    ?>
        <div id="colors-<?php echo $key ?>-settings" x-lazyload="lazyload--<?php echo $key ?>--list" x-space="colors" class="browse-modal-container opacity-0 hidden transition-all ease-in-out duration-200 flex flex-wrap items-start content-start w-full py-4">
